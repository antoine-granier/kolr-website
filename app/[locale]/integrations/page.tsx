...
export default function IntegrationsPage() {
  ...
  const steps = [
    { icon: <Palette size={24} />, title: t("step1Title"), desc: t("step1Desc"), color: "kolr-cyan" },
    { icon: <Download size={24} />, title: t("step2Title"), desc: t("step2Desc"), color: "kolr-purple" },
    { icon: <Layers size={24} />, title: t("step3Title"), desc: t("step3Desc"), color: "kolr-green" },
  ];

  const formats = [
    { icon: <Code size={18} />, name: t("featureCss"), color: "kolr-cyan" },
    { icon: <FileJson size={18} />, name: t("featureJson"), color: "kolr-purple" },
    { icon: <Copy size={18} />, name: t("featureHex"), color: "kolr-green" },
    { icon: <FileCode size={18} />, name: t("featureSvg"), color: "kolr-orange" },
    { icon: <Zap size={18} />, name: t("featureTailwind"), color: "kolr-cyan" },
    { icon: <FileCode size={18} />, name: t("featureScss"), color: "kolr-purple" },
  ];
      ...
      <div className="container max-w-[1100px]">
        ...
        <Reveal animation="reveal-up">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-kolr-surface border border-kolr-border rounded-full mb-6">
              ...
              <span className="text-sm font-bold text-kolr-text-muted">Figma · Sketch · More</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-4">{t("title")}</h1>
            <p className="text-lg text-kolr-text-muted max-w-[600px] mx-auto">{t("subtitle")}</p>
          </div>
        </Reveal>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-16">
          <Reveal animation="reveal-up" delay={1}>
            <div className="bg-kolr-surface border border-kolr-border rounded-2xl p-6 h-full hover:border-kolr-purple/50 transition-all duration-300">
              ...
              <p className="text-sm text-kolr-text-muted leading-relaxed mb-5">{t("figmaDesc")}</p>
              <div className="flex flex-col gap-2.5">
                {figmaFeatures.map((feature, i) => (
                  <div key={i} className="flex items-center gap-2.5">
                    <CheckCircle2 size={14} className="text-kolr-purple shrink-0" />
                    <span className="text-sm text-kolr-text-muted">{feature}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Reveal animation="reveal-up" delay={2}>
            <div className="bg-kolr-surface border border-kolr-border rounded-2xl p-6 h-full hover:border-kolr-orange/50 transition-all duration-300">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-xl bg-kolr-orange/10 flex items-center justify-center">
                  <svg width="24" height="22" viewBox="0 0 24 22" fill="none">
                    <path d="M5.25 0.5L12 0L18.75 0.5L24 8L12 22L0 8L5.25 0.5Z" fill="#FDB300"/>
                    <path d="M5.0625 8L12 22L0 8H5.0625Z" fill="#EA6C00"/>
                    <path d="M18.9375 8L12 22L24 8H18.9375Z" fill="#EA6C00"/>
                    <path d="M5.0625 8H18.9375L12 22L5.0625 8Z" fill="#FDAD00"/>
              ...
              <p className="text-sm text-kolr-text-muted leading-relaxed mb-5">{t("sketchDesc")}</p>
              <div className="flex flex-col gap-2.5">
                {sketchFeatures.map((feature, i) => (
                  <div key={i} className="flex items-center gap-2.5">
                    <CheckCircle2 size={14} className="text-kolr-orange shrink-0" />
                    <span className="text-sm text-kolr-text-muted">{feature}</span>
                  </div>
                ))}
              </div>
          ...
          {steps.map((step, i) => (
            <div key={i} className="contents">
              <div className="bg-kolr-surface border border-kolr-border rounded-2xl p-8 text-center relative flex flex-col items-center min-h-[250px]">
                <div className={`w-14 h-14 rounded-2xl bg-${step.color}/10 text-${step.color} flex items-center justify-center mb-5`}>
                  {step.icon}
                </div>
                ...
                <p className="text-sm text-kolr-text-muted leading-relaxed">{step.desc}</p>
              </div>
              {i < 2 && (
        ...
        <Reveal animation="reveal-up" delay={2}>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-16">
            {formats.map((fmt, i) => (
              ...
              >
                <div className={`w-9 h-9 rounded-lg bg-${fmt.color}/10 text-${fmt.color} flex items-center justify-center shrink-0`}>
                  {fmt.icon}
                </div>
        ...
        <Reveal animation="reveal-up" delay={1}>
          <div className="bg-gradient-to-br from-kolr-purple/10 via-kolr-cyan/5 to-kolr-green/10 border border-kolr-border rounded-2xl p-10 text-center">
            ...
            <p className="text-kolr-text-muted mb-6 max-w-[500px] mx-auto">{t("ctaDesc")}</p>
            <Link
