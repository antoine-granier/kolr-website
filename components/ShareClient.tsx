...
function ShareClientContent({ t, locale }: { t: Record<string, string>; locale: string }) {
          ...
          {displayColors.length === 0 && (
            <Reveal animation="reveal-up" delay={1}>
              <div className="bg-white/[0.03] border border-white/[0.08] rounded-[2rem] p-8 text-center mb-12 backdrop-blur-xl">
                ...
                />
                <p className="text-kolr-text-muted">{t["share.noColors"]}</p>
              </div>
            </Reveal>
          )}
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6 auto-rows-auto">
                  ...
                  {editColors.length < 8 && (
                    ...
                    >
                      <Plus size={20} />
                      <span className="text-xs mt-1">{t["share.addColor"]}</span>
                    </button>
                  )}
                </div>
...
export default function ShareClient({ t, locale }: { t: Record<string, string>; locale: string }) {
  return (
    <Suspense
      fallback={
        <div className="bg-kolr-bg text-white">
    ...
    >
      <ShareClientContent t={t} locale={locale} />
    </Suspense>
  );
}
