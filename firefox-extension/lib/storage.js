/**
 * Kolr Storage - browser.storage.local wrapper (Firefox)
 */

const KolrStorage = (() => {

  async function getPalettes() {
    const result = await browser.storage.local.get("kolr_palettes");
  ...
  async function savePalette(palette) {
    ...
    await browser.storage.local.set({ kolr_palettes: palettes });
  }

  async function deletePalette(id) {
    const palettes = await getPalettes();
    const filtered = palettes.filter(p => p.id !== id);
    await browser.storage.local.set({ kolr_palettes: filtered });
  }

  async function getRecentColors() {
    const result = await browser.storage.local.get("kolr_recent_colors");
  ...
  async function addRecentColor(hex) {
    ...
    await browser.storage.local.set({ kolr_recent_colors: colors });
  }
