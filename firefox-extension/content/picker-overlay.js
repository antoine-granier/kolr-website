...
(() => {
  ...
  hint.appendChild(makeSpan("Kolr", "color:#00f2ff;font-weight:800;"));
  hint.appendChild(makeSpan(" | ", "opacity:0.4;"));
  hint.appendChild(document.createTextNode("Click to pick "));
  hint.appendChild(makeSpan("·", "opacity:0.4;"));
  hint.appendChild(document.createTextNode(" Scroll to navigate "));
  hint.appendChild(makeSpan("·", "opacity:0.4;"));
  hint.appendChild(document.createTextNode(" Esc to cancel"));
  ...
  function requestNewScreenshot() {
    ...
    browser.runtime.sendMessage({ action: "capture-screenshot" }).then((response) => {
  ...
  function onClick(e) {
    ...
    browser.runtime.sendMessage({ action: "pick-result", color: hex });
    ...
    swatch.style.cssText = `width:18px;height:18px;border-radius:4px;background:${hex};border:2px solid rgba(0,0,0,0.2);flex-shrink:0;`;
    toast.appendChild(swatch);
    toast.appendChild(document.createTextNode(` Copied ${hex.toUpperCase()}`));
